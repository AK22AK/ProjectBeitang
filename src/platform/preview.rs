...
fn preview_cache_dir() -> Result<PathBuf, String> {
    ...
    let dir = base.join("robinne").join("preview");
...
fn export_attachment_to_cache(attachment: &Attachment, bytes: &[u8]) -> Result<PathBuf, String> {
...
fn open_with_system(path: &Path) -> Result<(), String> {
    ...
    let status = std::process::Command::new("cmd")
        ...
        .status()
        .map_err(|err| format!("调用系统默认应用失败 {}: {}", canonical_path.display(), err))?;
...
pub fn open_saved_attachment(
    ...
    if let Some(bytes) = file_data {
        let path = export_attachment_to_cache(attachment, &bytes)?;
